...
Chains between the two generic collections:
    ...
    acidnetworks.base_freebsd.site  ->  acidnetworks.backupbox.backupbox  ->  acidnetworks.hardening_freebsd.harden
...
`rest-server --append-only --private-repos`. A client can write new archives and
cannot prune old ones. rsync uses `rrsync -wo`, which is write-only.
...
| `jail_base` | Versioned read-only base dataset, weekly `pkg -j` updates |
...
They also have no reason to talk to anything else: `pkg -j` runs the *host's*
pkg against the jail root, so package installs use host networking. So no pf
